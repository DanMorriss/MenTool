...
class HomeView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Mood.objects.filter(
            user=self.request.user).order_by("-date")[:7][::-1]
        return queryset


# Create Mood page View
class MoodView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            "A mood record is successfully saved!",
            extra_tags="alert alert-success alert-dismissible",
        )
        return super().form_valid(form)


# Mood Detail page View
...
class UpdateMoodView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            "Successfully updated your Mood!",
            extra_tags="alert alert-success alert-dismissible",
        )
        return super().form_valid(form)
...
# Delete Page View - prepared for the future release
